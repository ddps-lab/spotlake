...
def get_binpacked_workload(filedate, bucket_name, s3_path_prefix):
    ...
    if s3_path_prefix and s3_path_prefix != "rawdata/aws": # Attempt to respect prefix if it's not the default rawdata path which might be for data
...
def main():
    ...
    # ------ Collect Spot Price ------
    try:
        workload = get_binpacked_workload(S3_DIR_NAME, BUCKET_NAME, S3_PATH_PREFIX)

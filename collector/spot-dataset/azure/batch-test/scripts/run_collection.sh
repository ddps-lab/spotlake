...
echo "Starting SpotLake Azure Data Collection (TEST) for timestamp: $TIMESTAMP"

# Run collection scripts in parallel
echo "Starting SPS Collection..."
python3 collector/spot-dataset/azure/batch-test/sps/collect_sps.py --timestamp "$TIMESTAMP" &
PID_SPS=$!

echo "Starting IF Collection..."
python3 collector/spot-dataset/azure/batch-test/if/collect_if.py --timestamp "$TIMESTAMP" &
PID_IF=$!

echo "Starting Price Collection..."
python3 collector/spot-dataset/azure/batch-test/price/collect_price.py --timestamp "$TIMESTAMP" &
PID_PRICE=$!
...
if [ $STATUS_SPS -eq 0 ] && [ $STATUS_IF -eq 0 ] && [ $STATUS_PRICE -eq 0 ]; then
    ...
    if [ -f /tmp/sps_key.txt ]; then
        ...
        python3 collector/spot-dataset/azure/batch-test/merge/merge_data.py --sps_key "$SPS_KEY"
        ...
        MONITORING_BASE_PATH="s3://spotlake-test/rawdata/azure/monitoring/${YEAR}/${MONTH}/${DAY}"
        ...
        aws s3 cp "$EXECUTION_FILE" "${MONITORING_BASE_PATH}/executionTime/"
        aws s3 cp "$MEMORY_FILE" "${MONITORING_BASE_PATH}/memory/"
        aws s3 cp "$INSTANCE_TYPE_FILE" "${MONITORING_BASE_PATH}/instanceType/"
    else

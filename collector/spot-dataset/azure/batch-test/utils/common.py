...
class S3Handler:
    ...
    def upload_file(self, data, file_path, file_type="json", set_public_read=False, bucket_name=None):
        target_bucket = bucket_name if bucket_name else STORAGE_CONST.WRITE_BUCKET_NAME
        try:
            ...
            self.client.upload_fileobj(file, target_bucket, file_path)

            if set_public_read:
                object_acl = self.resource.ObjectAcl(target_bucket, file_path)
                object_acl.put(ACL='public-read')

            print(f"[S3]: Succeed to upload. Filename: [{file_path}] to Bucket: [{target_bucket}]")

        except Exception as e:
            print(f"Upload failed for {file_path} to {target_bucket}: {e}")

    def read_file(self, file_path, file_type="json", bucket_name=None):
        target_bucket = bucket_name if bucket_name else STORAGE_CONST.READ_BUCKET_NAME
        try:
            response = self.client.get_object(Bucket=target_bucket, Key=file_path)
            ...
            print(f"Error reading {file_path} from S3 Bucket {target_bucket}: {e}")
            return None

...
def update_latest_price_saving_if(data, time_datetime):
    try:
        ...
        # Re-reading to check usage of wrappers, direct upload used in legacy but S3 wrapper supports it.
        # Actually legacy used S3Wrapper .upload_file or boto3 direct?
        # Legacy used boto3 direct upload_fileobj for detailed control (ACL public-read).
        # My S3Handler supports set_public_read. Using S3 wrapper is better for consistency.
        
        # NOTE: S3.upload_file expects data object, not file path.
        # For pkl.gz, it expects dataframe.
...
def save_raw_price_saving_if(data, time_datetime):
    try:
        ...
        # S3Handler supports df_to_csv.gz

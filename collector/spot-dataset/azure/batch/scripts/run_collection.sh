...
python3 collector/spot-dataset/azure/batch/sps/collect_sps.py --timestamp "$TIMESTAMP" &
PID_SPS=$!

echo "Starting IF Collection..."
python3 collector/spot-dataset/azure/batch/if/collect_if.py --timestamp "$TIMESTAMP" &
PID_IF=$!

echo "Starting Price Collection..."
python3 collector/spot-dataset/azure/batch/price/collect_price.py --timestamp "$TIMESTAMP" &
PID_PRICE=$!
...
if [ $STATUS_SPS -eq 0 ] && [ $STATUS_IF -eq 0 ] && [ $STATUS_PRICE -eq 0 ]; then
    ...
    if [ -f /tmp/sps_key.txt ]; then
        ...
        MONITORING_BASE_PATH="s3://spotlake/rawdata/azure/monitoring/${YEAR}/${MONTH}/${DAY}"
    ...
    # Even if some failed, we might want to clean up or alert?
    # For now, exit error.
    exit 1
fi

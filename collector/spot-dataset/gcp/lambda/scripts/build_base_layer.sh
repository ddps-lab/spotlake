...
LAYER_NAME="${LAYER_NAME:-gcp-collector-modules}"
RUNTIME="${RUNTIME:-python3.12}"
ARCHITECTURE="${ARCHITECTURE:-arm64}"
PACKAGES="${PACKAGES:-requests==2.32.5 google-auth}"
...
LAYER_DESCRIPTION="${LAYER_DESCRIPTION:-GCP collector base modules (${RUNTIME}, ${ARCHITECTURE})}"
...
build_dir="$(mktemp -d "${LAMBDA_DIR}/.build/base-layer-${RUNTIME}-${ARCHITECTURE}-XXXXXX")"
...
echo "Built base layer zip: ${OUTPUT_ZIP}"
...
if [ "${PUBLISH_LAYER}" = "1" ]; then
  ...
  echo "Published base layer: ${layer_arn}"
  echo "Set this ARN as base_layer_arn in the test tfvars."
fi
